...
class QueryBuilder(ObservesEvents):
    ...
    def update(
        ...
        Arguments:
            ...
            ignore_mass_assignment {bool, optional}: Whether the update should ignore mass assignment
    ...
    def count(self, column=None, dry=False):
        ...
        if not column:
            result = self.get_connection().query(self.to_qmark(), self._bindings, results=1)
    ...
    def _register_relationships_to_model(self, related, related_result, hydrated_model, relation_key):
        ...
        Args:
            ...
            hydrated_model (Model|Collection): If a collection we will need to loop through the collection of
                                                nodels and register each one individually.
                                                Else we can just load the related_result into the
                                                hydrated_models
    ...
    def all(self, selects=[], query=False):
        ...
        result = self.get_connection().query(self.to_qmark(), self._bindings) or []
